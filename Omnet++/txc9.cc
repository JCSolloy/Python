...
class Tic9 : public cSimpleModule
{
  ...
  public:
    virtual ~Tic9();//destructor to delete the message and cancel the timeout event

  protected:
    virtual cMessage *generateNewMessage();
    // generate a new message with a different name every time
    virtual void sendCopyOf(cMessage *msg); //send a copy of the message to the output gate
...
cMessage *Tic9::generateNewMessage()
{
    ...
    char msgname[20];
    sprintf(msgname, "tic-%d", ++seq);// tic-1, tic-2, tic-3, ...
    // ++seq devuelve el valor de seq incrementado en 1 y luego lo incrementa
